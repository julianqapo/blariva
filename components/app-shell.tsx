...
// ── Scoped theme context (no next-themes, no hydration issues) ──
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`${theme === 'dark' ? 'dark' : 'light-override'} h-screen w-full`}>
      <div className="h-full w-full bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-300 flex relative transition-colors duration-500">
        {/* Ambient brand glow */}
        ...
        <div className="flex-1 flex flex-col relative z-10 overflow-hidden">
          ...
          <header className="h-20 border-b border-slate-200 dark:border-white/[0.04] bg-white/60 dark:bg-slate-950/30 backdrop-blur-xl flex items-center justify-between px-8 transition-colors duration-500">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={toggle} className="w-9 h-9 rounded-full flex items-center justify-center text-slate-500 hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-500/10 transition-colors">
